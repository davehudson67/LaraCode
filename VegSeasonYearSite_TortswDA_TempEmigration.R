...
code <- nimbleCode( {
  ...
  for(t in 1:nyears){
    ...
    for(k in 1:nseasons){
      ...
      psi[k, t] <- sum(lambda[1:nsites, t]) / nind
      #psiS[k, t] ~ dunif(0, 1)  
      ...
      for(i in 1:nind){                                  # i is index for individuals
        ...
        mu[i, k, t] <- (z[i, k, t] * (available[i, k, t])) * p[i, k, t]                 # 'straw man' for WinBUGS
        ...
        available[i, k, t] ~ dbern(available_prob[k])
      } # individual
    } # season
...
saveRDS(run, "Outputs/VegSeasonYearSite_TE_100K.rds")
