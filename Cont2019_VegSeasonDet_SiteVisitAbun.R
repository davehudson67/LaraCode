...
#sitesvisited <- as.numeric(unique(data_aug$SectionCode))
y <- data_aug$y
nind <- sum(y)
nz <- length(y) - nind
...
## Site vistsed indiciator 
visited_sites <- unique(torts2019$SectionCode)
sitesvisited <- rep(0, 379)
all_sites <- levels(torts2019$SectionCode)  # Replace this with your actual list of all SectionCodes

# Now, mark sites that have been visited
sitesvisited[all_sites %in% visited_sites] <- 1
...
code <- nimbleCode({
  ...
  for(k in 1:nseason){
    alpha0[k] ~ dnorm(0, 0.1)  # Intercept of log(sigma) for each season
  }
  ...
  for(s in 1:nsites){                    # s is index for sites
    ...
    log(lambda[s]) <- beta0 * sitesvisited[s]
...
config$addMonitors(c("psi", "N", "Ntotal", "alpha", "alpha0", "beta0"))
config
...
parms <- c("psi", "Ntotal", "alpha", "alpha0", "beta0")
mcmcplot(run$samples, parms = parms)
