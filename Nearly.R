...
#d <- array(runif(10000, 0, 50), dim = c(2, 5000))
#y <- array(rbinom(1000, 1, 0.5), dim = c(2, 5000))
#site <- array(sample(1:263, 10000, replace = TRUE), dim = c(2, 5000))
#nind <- c(5000, 5000)
#nz <- c(5000, 5000)
...
consts <- list(nsites=sites_season, VegTypeA = VegTypeA, nVegTypes = nVegTypes, B=B, nind=nind, 
               nz=nz, site=site, nseason=nseason)
# Inits
zst <- y
d_init <- d
