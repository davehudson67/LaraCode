...
code <- nimbleCode( {
  ...
  for(k in 1:nseasons){
    ...
    for(i in 1:5000){                       # i is index for individuals
      ...
      p[i, k] <- exp(-x[i, k] * x[i, k] / (2 * sigma[site[i, k]] * sigma[site[i, k]])) # Det. function
...
config$addMonitors(c("alpha0", "alpha", "psi", "psiS", "Ntotal2019_W", "Ntotal2019_D", "beta", "beta0"))
config
...
saveRDS(run, "Outputs/2019WD_VegSeason2.rds")
run$summary
