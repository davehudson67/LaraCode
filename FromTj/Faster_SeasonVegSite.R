...
code <- nimbleCode({
  ...
  for(s in 1:nsite_ind) {
    ...
    ## priors
    lsigma[s] <- alpha1[season_ind[s]] + alpha[VegTypeM[sitea_ind[s]]] + beta[season_ind[s] + 1, VegTypeM[sitea_ind[s]]]
    sigma[s] <- exp(lsigma[s])
  }
  for(v in 1:nVegType) {
    alpha[v] ~ dnorm(0, sd = 100)
  }
  
  for(seas in 1:nSeasons) {
    alpha1[seas] ~ dnorm(0, sd = 100)
  #  for(j in 1:nVegType) {
  #    beta[seas, j] ~ dnorm(0, sd = 100)
  #  }
  }
...
inits <- function(nVegType, M, aug, model, nsites) {
  ...
  while(valid == 0) {
    inits1 <- list(
      ...
      alpha1 = rnorm(2, 0, 1),
      #beta = matrix(rnorm(16, 0, 1), nrow = 2),
...
#out1$summary
mcmcplot(out1, parms = c("N", "alpha1", "alpha"))
