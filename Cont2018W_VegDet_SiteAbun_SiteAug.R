...
nind <- nrow(tortsW2018)

#site_visits <- table(tortsW2018$SectionCode)
sites_visited <- levels(tortsW2018$SectionCode)[table(tortsW2018$SectionCode) > 0]
...
aug_site_codes <- factor(aug_site, levels = levels(tortsW2018$SectionCode))
aug_dist <- rep(NA, nz)

## Combine data
site <- as.numeric(factor(c(tortsW2018$SectionCode, aug_site_codes), levels = levels(tortsW2018$SectionCode)))
